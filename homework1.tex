...
& \Theta(1): [\pi],
& \quad \Theta(\lg^*2^n): [\lg^*2^n],
& \quad \Theta(e^{2n}): [e^{2n}],\\
& \Theta(n^{1337}): [n^{1337}],
& \quad \Theta(\lg^*{n/2}): [\lg^*{n/2}],
& \quad \Theta(2^{\lg^*{n}}): [2^{\lg^*{n}}],\\
& \Theta({\lg n}^{\lg^*{n}}): [{\lg n}^{\lg^*{n}}],
\end{aligned}
\right\}
\]\\

before simplifying further, let's order this array from smallest
...
is $\Theta(\lg^*n)$. We have four relevant Theta functions here: 
...
while we do have the tight bound $\Theta({\lg n}^{\lg^*n})$,
this function is clearly $\Omega(\lg n)$ due to the fact that even 
if we treat $lg^*n$ as a constant value, it will still be a constant
greater than 1. 
